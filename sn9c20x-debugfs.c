 ...
 */
static const char *debug_dir_name = "sn9c20x";
...
static int bridge_dump_show(struct seq_file *m, void *v)
{
	...
	struct usb_sn9c20x *dev = m->private;

	if (usb_sn9c20x_control_read(dev, reg, values, 16) < 0)
		return -EACCES;
...
int bridge_value_get(void *data, __u64 *val)
{
	...
	if (dev != NULL) {
		if (dev->debug.bridge_addr >= 0x1000 &&
		    dev->debug.bridge_addr <= 0x11ff) {
			if (usb_sn9c20x_control_read(dev,
						      dev->debug.bridge_addr,
						      &value, 1) >= 0) {
				*val = value;
			}
...
int sensor_value8_get(void *data, __u64 *val)
{
	...
	__u8 value;
	if (dev != NULL) {
		if (sn9c20x_read_i2c_data(dev, 1,
					  dev->debug.sensor_addr,
					  &value) >= 0) {
			*val = value & 0xFF;
		}
...
int sensor_value16_get(void *data, __u64 *val)
{
	...
	__u8 buf[2];
	if (dev != NULL) {
		if (sn9c20x_read_i2c_data(dev, 2,
					  dev->debug.sensor_addr,
					  buf) >= 0) {
...
int sensor_value32_get(void *data, __u64 *val)
{
	...
	__u8 buf[4];
	if (dev != NULL) {
		if (sn9c20x_read_i2c_data(dev, 4,
					  dev->debug.sensor_addr,
					  buf) >= 0) {
 ...
 */
int sn9c20x_create_debugfs_files(struct usb_sn9c20x *dev)
{
	char device_name[9];
	if (debug_dir) {
 ...
 * @returns 0 if all is OK
 */
int sn9c20x_remove_debugfs_files(struct usb_sn9c20x *dev)
{
